...
public abstract class BaseEntryPointSelector implements EntryPointSelector {
	...
	public void writeSelectionCode(JClassType classType, PrintWriter pw) {
		String entryPoint = classType.getQualifiedSourceName();
		pw.println("		if (" + generateMatchCondition(classType) + ") {");
		pw.println("			" + EntryPointFactoryImplGenerator.ENTRY_POINT_VAR + " = \"" + entryPoint + "\";");
		pw.println("		}");
	}
